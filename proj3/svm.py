...
class Classifier(object):
    ...
    def classification(self, train_feat, test_feat):
        ...
        # print("Training a GaussianNB Classifier.")
        print("Training a LinearRegression Classifier.")
        # print("\nTraining a Kernel SVM Classifier.")
        print('Shape of the train data is:', train_feat.shape)
        clf.fit(train_feat[:, :-1], train_feat[:, -1])
        # 保存模型
        with open('caches/linearreg.pkl', 'wb') as f:
            pickle.dump(clf, f)
        ...
        t1 = time.time()
        print('\nThe testing classification accuracy is %f' % rate)
